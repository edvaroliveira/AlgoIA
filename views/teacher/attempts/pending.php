<?php
$pageTitle = 'Correções pendentes — Docente';
$attempts = $attempts ?? [];
$filters = $filters ?? ['search' => '', 'from_date' => '', 'to_date' => '', 'min_age_hours' => 0];
$pagination = $pagination ?? ['totalItems' => count($attempts), 'totalPages' => 1, 'currentPage' => 1, 'path' => '/teacher/attempts/pending', 'query' => $filters];
global $session;
?>
...
<div class="page-header">
  <div>
    ...
    <p class="subtitle">Tentativas dos seus exercícios que ficaram aguardando reprocessamento da avaliação automática.</p>
  </div>
  <a href="<?= \Core\app_url('/teacher/dashboard') ?>" class="btn btn--ghost">Voltar ao painel</a>
</div>

<section class="card card--narrow">
  <div class="card-body">
    <form method="GET" action="<?= \Core\app_url('/teacher/attempts/pending') ?>" class="form">
      <div class="form-row">
        <div class="form-group">
          <label class="form-label" for="pending-search">Buscar</label>
          <input id="pending-search" type="text" name="search" class="form-input" value="<?= \Core\View::e($filters['search'] ?? '') ?>" placeholder="Aluno, e-mail, exercício ou turma">
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group" style="justify-content: flex-end;">
          ...
          <div class="td-actions">
            ...
            <a href="<?= \Core\app_url('/teacher/attempts/pending') ?>" class="btn btn--ghost">Limpar</a>
          </div>
        </div>
      </div>
    </form>
